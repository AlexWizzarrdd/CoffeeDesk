...
const calculateMaskParams = (wrapperElement: EventTarget & HTMLDivElement, maskRef: RefObject<SVGCircleElement|null>) => {
...
const linkHandler = (ev: React.MouseEvent<HTMLDivElement, MouseEvent>, circleRef: RefObject<SVGCircleElement | null>) => {
...
export const Sidebar = () => {
    ...
    useEffect(() => {
        updateMaskPosition(circleRef, calculateMaskParams(document.querySelector('.active')?.parentNode as HTMLDivElement, circleRef))
    }, [])
            ...
            <svg className='nav-mask'>
                <defs>
                    <mask id="circle-mask">
                        ...
                        { window.innerWidth < 1026 ? null : <circle ref={circleRef} r="90" fill='white' className="nav-mask-circle"/> }
                    </mask>
                </defs>
            </svg>
        </div>
        <nav className='nav'>
            <div key={'profileLink'} className='nav-link-wrapper' onMouseDown={(ev) => linkHandler(ev, circleRef)}>
                <NavLink to='/'>
                    <ProfileIcon />
                </NavLink>
            </div>
            <div key={'calendarLink'} className='nav-link-wrapper' onMouseDown={(ev) => linkHandler(ev, circleRef)}>
                <NavLink to='/calendar'>
                    <CalendarIcon />
                </NavLink>
            </div>
            <Activity mode={user.role === 'manager' ? 'visible' : 'hidden'}>
                <div key={'usersLink'} className='nav-link-wrapper' onMouseDown={(ev) => linkHandler(ev, circleRef)}>
                    <NavLink to='/users'>
                        <ProfileIcon />
                    </NavLink>
                </div>
